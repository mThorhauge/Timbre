...
void setup()
{
  ...
  // very fast tempos.
  kick  = new Sampler( "pig.wav", 4, minim );
  snare = new Sampler( "pig.wav", 4, minim );
  hat   = new Sampler( "pig.wav", 4, minim );
...
void keyReleased()
{
  if(key == '+')
  {
    kick.setSampleRate(kick.sampleRate()*1.05);
    snare.setSampleRate(snare.sampleRate()*1.01);
    hat.setSampleRate(hat.sampleRate()*1.01);
  }
  if(key == '-')
  {
    kick.setSampleRate(kick.sampleRate()/1.05);
    snare.setSampleRate(snare.sampleRate()/1.05);
    hat.setSampleRate(hat.sampleRate()/1.01);
  }
}